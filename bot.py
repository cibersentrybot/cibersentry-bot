...
# --- TUS FUNCIONES TÉCNICAS (IGUALES A LAS TUYAS) ---
...
async def manejar_contenido(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input.lower().startswith("escanear"):
        ...
        try:
            ...
            # Aquí te dirá el error de la API si la clave falla
            await update.message.reply_text(f"⚠️ Error de IA: {e}")
...
if __name__ == '__main__':
    # Arrancamos Flask en un hilo para que Render no nos corte la conexión
    threading.Thread(target=run_flask, daemon=True).start()

    if TELEGRAM_TOKEN:
        print("✅ BOT ARRANCANDO...")
        # Usamos la forma estándar de arrancar para evitar conflictos de bucle
        app_bot = ApplicationBuilder().token(TELEGRAM_TOKEN).build()
        app_bot.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), manejar_contenido))
        app_bot.run_polling()
    else:
